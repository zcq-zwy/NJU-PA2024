...
void fce_init();
void fce_run();
extern bool fce_draw_enabled;
extern uint32_t fce_canvas[SCR_W * SCR_H];
...
static inline void draw_fast(int x, int y, int idx) {
  fce_canvas[y * SCR_W + x] = palette[idx];
}

static inline void draw(int x, int y, int idx) {
  if (fce_draw_enabled && (unsigned)x < SCR_W && (unsigned)y < SCR_H) {
    draw_fast(x, y, idx);
  }
}
