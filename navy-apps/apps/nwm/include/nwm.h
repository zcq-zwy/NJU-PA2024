...
protected:
  ...
  void draw_raw_ch(BDF_Font *font, int x, int y, char ch, uint32_t color);
...
public:
  ...
  int fw, fh, dx, dy; // frame buffer size (fw * fh) and offset (dx, dy)
  ...
  Window(WindowManager *wm, const char *cmd, const char * const *argv, const char **envp);
  ~Window();
  ...
  void draw_ch(BDF_Font *font, int x, int y, char ch, uint32_t color);
  void draw();
...
private:
  ...
  void draw_px(int x, int y, uint32_t col, bool has_alpha);
  void draw_window(Window *win);

public:
  int w, h, tw, th;
  BDF_Font *font, *title_font;
