...
if [ "$#" -lt 1 ]; then
  echo "用法: $0 'echo hi' 'ls' 'symlinktest' 'bigfile 1'" >&2
  exit 1
fi
...
echo "[fs-cmd] 清理旧的 NEMU 进程"
pkill -f riscv32-nemu-interpreter 2>/dev/null || true

cd "$XV6_HOME"
if [ "$NO_REBUILD" = "1" ]; then
  echo "[fs-cmd] 跳过重建，复用当前 kernel/fs.img"
else
  echo "[fs-cmd] 强制重建 xv6"
  rm -f fs.img user/_bigfile user/bigfile.o user/_symlinktest user/symlinktest.o kernel/kernel kernel/kernel.bin
...
while True:
    if time.time() > deadline:
        proc.terminate()
        raise SystemExit("超时: xv6 命令执行未完成")
