...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal   , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; if (rd == 1 || rd == 5) ftrace_call(s->pc, s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr  , I, word_t t = src1 + imm; R(rd) = s->snpc; s->dnpc = t & ~1; if (rd == 0 && BITS(s->isa.inst, 19, 15) == 1 && imm == 0) ftrace_ret(s->pc, s->dnpc); else if (rd == 1 || rd == 5) ftrace_call(s->pc, s->dnpc));
