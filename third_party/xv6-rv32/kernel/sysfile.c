...
{
  struct file *f;
  uint32 st; // user pointer to struct stat
...
static struct inode *
follow_symlink(struct inode *ip, int omode)
{
  int depth;
  char path[MAXPATH];
  struct inode *next;
  ...
  for(depth = 0; depth < 10; depth++){
    if(ip->type != T_SYMLINK)
      return ip;
    if(readi(ip, 0, (uint32)path, 0, sizeof(path)) < 0){
      iunlockput(ip);
      return 0;
    }
    iunlockput(ip);
...
sys_open(void)
{
  ...
  if(omode & O_CREATE){
    ...
    ilock(ip);
    if((ip = follow_symlink(ip, omode)) == 0){
      end_op();
      return -1;
    }
...
sys_symlink(void)
{
  ...
  len = strlen(target) + 1;
  if(writei(ip, 0, (uint32)target, 0, len) != len){
    iunlockput(ip);
    end_op();
    return -1;
  }
